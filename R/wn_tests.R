...
ljung_box <- function(x, p = 0, q = 0, k_val = c(24,48), alpha = 0.05) {
  ...
  rvData = data.frame(tests) %>% 
    t() %>%
    data.frame() %>% 
    tibble::remove_rownames() %>%
    dplyr::mutate(Decision = ifelse(.$pval < alpha, "REJECT NULL", "FTR NULL"))
...
#' 
#' @export
white_noise_eval = function(x, p = 0, q = 0, k_val = c(24,48), alpha = 0.05, lag.max = 50){
  ...
  ljung_box(x, p = p, q = q, k_val = k_val, alpha = alpha)
}
